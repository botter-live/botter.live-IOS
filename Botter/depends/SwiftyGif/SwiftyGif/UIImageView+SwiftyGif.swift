...
public extension UIImageView {
    ...
    private func parseDownloadedGif(url: URL,
        ...
        do {
            ...
            setGifImage(image, manager: manager, loopCount: loopCount)
            startAnimatingGif()
            gifDelegate?.gifURLDidFinish?(sender: self)
        } catch {
            report(url: url, error: error)
        }
    }
    ...
    private func report(url: URL, error: Error?) {
        gifDelegate?.gifURLDidFail?(sender: self, url: url, error: error)
    }
}
    ...
    private func updateIndex() {
        ...
        if syncFactor == 0, let imageCount = gif.imageCount, imageCount > 0 {
            ...
            if displayOrderIndex == 0 {
                if loopCount == -1 {
                    gifDelegate?.gifDidLoop?(sender: self)
                } else if loopCount > 1 {
                    gifDelegate?.gifDidLoop?(sender: self)
                    loopCount -= 1
                } else {
                    gifDelegate?.gifDidStop?(sender: self)
                    loopCount -= 1
                }
            }
        }
...
public extension UIImageView {
    ...
    var gifDelegate: SwiftyGifDelegate? {
    ...
    private var isPlaying: Bool {
        ...
        set {
            ...
            if newValue {
                gifDelegate?.gifDidStart?(sender: self)
            } else {
                gifDelegate?.gifDidStop?(sender: self)
            }
        }
